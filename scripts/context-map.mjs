...
/** Protocol schema version of the v3 directory + content-file layout. */
...
// ── v3 semantic sections ─────────────────────────────────────────────────────
// v3 keeps the eight fixed sections but renames two keys: "goal" → "goals"
// and "knowledge" → "notes". Display labels (and their localizations) are
// shared with v2.

export const V3_SECTION_KEYS = Object.freeze([
  "goals",
  "status",
  "tasks",
  "decisions",
...
const V3_TO_V2_KEY = Object.freeze({
  goals: "goal",
  status: "status",
  tasks: "tasks",
  decisions: "decisions",
...
export const V3_SECTION_LABELS = Object.freeze(
  Object.fromEntries(V3_SECTION_KEYS.map((key) => [key, SECTION_LABELS[V3_TO_V2_KEY[key]]]))
);

const V3_LABEL_TO_KEY = new Map();
for (const [key, labels] of Object.entries(V3_SECTION_LABELS)) {
  for (const label of Object.values(labels)) {
    V3_LABEL_TO_KEY.set(label.trim().toLowerCase(), key);
  }
}

/** Resolve a (possibly localized) section heading to its v3 semantic key. */
export function v3SectionKeyForLabel(label) {
  return V3_LABEL_TO_KEY.get(String(label).trim().toLowerCase()) || null;
}
...
export function nodeFingerprint(text, checked = false) {
...
// In v3 the Context Map is a compact semantic directory: each node line owns
// only a stable ID, a label, hierarchy, task state, and compact metadata
// (task priority / risk severity). Full node content lives in the content/
// files keyed by ID (see scripts/handoff-state.mjs).

const V3_ID_TOKEN_RE = /^`([^`]+)`\s*/;
const V3_META_RE = /^\*\*(high|medium|low)\*\*\s+/i;

export function emptyContextMapV3() {
  const sections = {};
  for (const key of V3_SECTION_KEYS) sections[key] = [];
  return { sections, extras: [] };
}

/**
 * Parse a v3 context-map Markdown directory. Returns null when the content is
 * empty or contains no recognized section heading (callers treat null as
 * "not a v3 map"). Nodes carry: id (null when the user has not been assigned
 * one yet), label, origin, depth, plus checked/priority for tasks and
 * severity for risks.
 */
export function parseContextMapV3(content) {
  if (!content || !String(content).trim()) return null;
  ...
  for (const rawLine of String(content).split("\n")) {
    ...
    const heading = trimmed.match(/^##\s+(.+?)\s*$/);
    if (heading && !trimmed.startsWith("###")) {
      currentKey = v3SectionKeyForLabel(heading[1]);
      if (currentKey) {
        recognized++;
        currentExtra = null;
        indentationLevels = [0];
    ...
    if (currentKey) {
      ...
      const node = { id: null, origin, depth };

      if (currentKey === "tasks") {
        const cb = text.match(/^\[([ xX])\]\s+(.*)$/);
        node.checked = cb ? cb[1].toLowerCase() === "x" : false;
        if (cb) text = cb[2].trim();
      }

      const idMatch = text.match(V3_ID_TOKEN_RE);
      if (idMatch) {
        node.id = idMatch[1];
        text = text.replace(V3_ID_TOKEN_RE, "").trim();
      }

      const meta = text.match(V3_META_RE);
      if (meta) {
        if (currentKey === "tasks") node.priority = meta[1].toLowerCase();
        if (currentKey === "risks") node.severity = meta[1].toLowerCase();
        if (currentKey === "tasks" || currentKey === "risks") {
          text = text.replace(V3_META_RE, "").trim();
        }
      }
      ...
      if (
        hasAgentMarker &&
        (!storedHash || storedHash === nodeFingerprint(node.label, !!node.checked))
      ) {
        node.origin = "agent";
      }
      map.sections[currentKey].push(node);
...
// ── v3 directory rendering ───────────────────────────────────────────────────

export function renderContextMapV3(map, opts = {}) {
  const lang = opts.lang && V3_SECTION_LABELS.goals[opts.lang] ? opts.lang : "en";
  const chunks = [
    "# Context Map\n\n" +
      `<!-- handoff-protocol:v${V3_PROTOCOL_VERSION} — Semantic directory: stable IDs, ` +
      "labels, hierarchy, and lightweight state only. Full node content lives in " +
      "the content/ files keyed by node ID. -->",
  ];

  for (const key of V3_SECTION_KEYS) {
    const lines = [`## ${V3_SECTION_LABELS[key][lang]}`, ""];
    for (const node of map.sections[key] || []) {
      let text = "";
      if (key === "tasks") text += `[${node.checked ? "x" : " "}] `;
      if (node.id != null) text += `\`${node.id}\` `;
      if (key === "tasks" && node.priority) text += `**${node.priority}** `;
      if (key === "risks" && node.severity) text += `**${node.severity}** `;
      text += node.label;
      const marker = node.origin === "agent"
        ? ` ${AGENT_MARKER} <!-- agent-hash:${nodeFingerprint(node.label, !!node.checked)} -->`
        : "";
      const indent = "  ".repeat(Math.max(0, Number(node.depth) || 0));
      lines.push(`${indent}- ${text}${marker}`);
    }
  ...
  chunks.push(`---\n\n*Generated by Handoff Protocol v${V3_PROTOCOL_VERSION}*`);
  return chunks.join("\n\n") + "\n";
}
