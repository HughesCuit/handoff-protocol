...
test("standalone template exposes the view controls and panes", async () => {
  const html = await source("web/standalone.html");
  for (const id of [
    "view-modes",
    "mode-tree",
    "mode-map",
    ...
    assert.match(html, new RegExp(`id="${id}"`), `web/standalone.html: ${id}`);
  }
